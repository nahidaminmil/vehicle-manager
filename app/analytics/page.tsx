...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      else processData(vehicles || [], sortMap, locations) // Pass dynamic locations
      setLoading(false)
    }
    fetchStats()
  }, [])

  function processData(vehicles: any[], sortMap: any, locations: string[]) {
    ...
    vehicles.forEach(v => {
      ...
      if (!typeGroups[typeName]) {
        ...
        // Initialize specific TOBs from fetched list
        locations.forEach(t => { 
      ...
      // --- TOB AGGREGATION ---
      // Safety check: if a vehicle belongs to a deleted/old location
      if (!g.tobs[tob]) {
          g.tobs[tob] = { 
            status: { active: 0, inactive: 0, maintenance: 0 },
            opCat: { fmc: 0, degraded: 0, nmc: 0 }
              ...
              <table className="w-full text-xs md:text-sm text-left">
                <thead>
                  <tr className="bg-gray-100 text-gray-500 uppercase text-[10px] font-extrabold tracking-wider border-b border-gray-200">
                    ...
                    <th colSpan={3} className="px-2 py-3 text-center bg-blue-50/50 border-l border-r border-gray-200 text-blue-800">Vehicle Status</th>
                    <th colSpan={3} className="px-2 py-3 text-center bg-purple-50/50 text-purple-800">Operational Category</th>
                  </tr>
                  ...
                  {tobList.map(tob => {
                    ...
                    return (
                      <tr key={tob} className="hover:bg-blue-50/30 transition-colors">
                        ...
                        <td className="px-2 py-3 text-center font-bold text-green-700 border-l border-gray-100 bg-green-50/30">{data.status.active || '-'}</td>
                        <td className="px-2 py-3 text-center font-bold text-red-700 bg-red-50/30">{data.status.inactive || '-'}</td>
                        <td className="px-2 py-3 text-center font-bold text-orange-600 bg-orange-50/30 border-r border-gray-100">{data.status.maintenance || '-'}</td>
                        <td className="px-2 py-3 text-center font-bold text-green-700">{data.opCat.fmc || '-'}</td>
                        <td className="px-2 py-3 text-center font-bold text-yellow-600">{data.opCat.degraded || '-'}</td>
                        <td className="px-2 py-3 text-center font-bold text-red-700">{data.opCat.nmc || '-'}</td>
                      </tr>
                    )
                  })}
                </tbody>
