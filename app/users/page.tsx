            ...
            {users.map((u) => (
                <div key={u.id} className={`bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between items-center transition-all ${editingId === u.id ? 'ring-2 ring-blue-500 bg-blue-50' : ''}`}>
                    ...
                    <div className="flex gap-2 min-w-fit">
                        {editingId === u.id ? (
                            <>
                                <button onClick={() => setEditingId(null)} className="flex items-center px-3 py-2 bg-gray-200 hover:bg-gray-300 rounded font-bold text-sm text-gray-700">
                                    <X className="w-4 h-4 mr-1"/> Cancel
                                </button>
                                <button onClick={saveChanges} className="flex items-center px-3 py-2 bg-green-600 hover:bg-green-700 rounded font-bold text-sm text-white">
